...
function ChipGenerator(props) {
    const { beniPropObj } = props;
    let beniProp = beniPropObj.map(i => i.name + ' ' + i.asset + ' ' + i.proportion)
    console.log(beniProp)
    // assetClients = ['Kamala Srinivas', 'Kamala']
    let temp = beniProp.map((beniPropi) => 
    ...
    {beniPropi}%
    <button type="button" className="inline-flex items-center p-0.5 ml-2 text-sm text-blue-400 bg-transparent rounded-sm hover:bg-blue-200 hover:text-blue-900 dark:hover:bg-blue-800 dark:hover:text-blue-300" data-dismiss-target="#badge-dismiss-default" aria-label="Remove">
